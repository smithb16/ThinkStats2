...
def HeightToCm(feet, inches):
    """Converts height of 'feet' 'inches' to cm.
    """
    inches += 12*feet
    return inches * 2.54
...
if __name__ == '__main__':
    ...
    ## Calculate percentage of BlueMan group applicants
    h_min = HeightToCm(5,10)
    h_max = HeightToCm(6,1)
    male_ht = df.loc[df.sex==1, 'htm3']
    mu = male_ht.mean()
    sigma = male_ht.std()

    dist = scipy.stats.norm(loc=mu, scale=sigma)
    bmg = dist.cdf(h_max) - dist.cdf(h_min)
    print('US men qualified for Blue Man Group: ',bmg)
